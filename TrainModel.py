...
# 註冊資料集到detectron2
register_coco_instances("alldata", {}, "./AllData/AllData(AddChecked).json", "./AllData/Labeled(AddChecked)")
...
def trainModel():
    ...
    # cfg.MODEL.WEIGHTS = os.path.join(cfg.OUTPUT_DIR, "model_final-2021-03-30(20210324+new100).pth") # 讀取已訓練過的權重
    cfg.SOLVER.IMS_PER_BATCH = IMS_PER_BATCH # 每個 iteration 看兩張圖片
    cfg.SOLVER.BASE_LR = 0.002 # 0.02 # learning rate
    cfg.SOLVER.MAX_ITER = int(len(dataset_dicts_test) / IMS_PER_BATCH) * 40  # number of iteration: 1 Epoch = MAX_ITER * BATCH_SIZE / TOTAL_NUM_IMAGES
